...
@IBDesignable public class DayNightSwitch: UIView {
    ...
    @IBInspectable public var on: Bool = false {
        didSet {
            ...
            if let c = self.changeAction {
                
                c(self.on)
                NotificationCenter.default.post(name: NSNotification.Name(rawValue: "dayThemeOn"), object: self.on)
            }
                ...
                UIView.animate(withDuration: 0.4, delay: 0, usingSpringWithDamping: 1, initialSpringVelocity: 0, options: UIViewAnimationOptions.allowUserInteraction, animations: {
                    ...
                    if let all = self.stars {
                        all.enumerated().forEach({ (i, star) in
                            ...
                            //dispatch_after(DispatchTime.now(DispatchTime.now(), Int64(0.1 * Double(i) * Double(NSEC_PER_SEC))), 
                            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1 * Double(i) * Double(NSEC_PER_SEC)) {
                                ...
                                DispatchQueue.main.asyncAfter(deadline: .now() + 0.05 * Double(NSEC_PER_SEC)) {
    ...
    override public func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        moved = true
        proccess(touches: touches, withEvent: event)
    }
    
    override public func touchesEnded(_ touches: Set<UITouch>,with event: UIEvent?) {
        self.proccess(touches: touches, withEvent: event)
    ...
    override public func touchesCancelled(_ touches: Set<UITouch>?,with event: UIEvent?) {
        self.touchesEnded(touches ?? Set<UITouch>(), with: event)
    }
